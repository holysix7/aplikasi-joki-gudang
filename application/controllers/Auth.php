...
class Auth extends CI_Controller {
    ...
    public function login(){
        ...
        if($check->num_rows() > 0){
            ...
            if($dataUser == "superadmin"){
                redirect('dashboard');
                $this->session->set_flashdata('alert_message', show_alert('<b class="text-success"><i class="fa fa-check-circle"></i></b>Berhasil Login','success'));
            }elseif($dataUser == "karyawan"){
                $this->session->set_flashdata('alert_message', show_alert('<b class="text-success"><i class="fa fa-check-circle"></i></b>Berhasil Login','success'));
            ...
            $this->session->set_flashdata('alert_message', show_alert('<i class="fa fa-times"></i> Username Atau Password Salah!','danger'));
            redirect('');
        }
    }
    ...
    public function logout(){
        if($this->session->sess_destroy()){
            ...
            $this->session->set_flashdata('alert_message', show_alert('<i class="fa fa-times"></i> Gagal Logout','danger'));
		}
        redirect('');
