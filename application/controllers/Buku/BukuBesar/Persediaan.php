...
class Persediaan extends CI_Controller
{
  ...
  public function index(){
    ...
    $jurnal     = $this->m_jurnal->get_data_buku_persediaan();
    $saldo_akhir  = 0;
    $gudang     = $this->m_stok->get_data();
    $dataGudang = [];
    $saldoTotalGudang = 0;
    foreach ($gudang as $rowGudang) {
      $saldoTotalGudang += $rowGudang['unit_price'] * $rowGudang['jumlah_stok'];  
    }
    // var_dump($saldoTotalGudang); die;
    $dataJurnal = [];
    ...
    $this->load->view('Buku/BukuBesar/Persediaan', compact('name', 'dataJurnal', 'saldo_kas', 'saldo_akhir', 'saldoTotalGudang'));
  }
